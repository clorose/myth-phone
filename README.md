...
현재 메시지와 버블톡은 서로 분리된 JSON 더미 데이터를 사용한다. 입력한 메시지는 저장되거나 다른 사용자에게 전송되지 않으며 새로고침하면 사라진다. 전화는 GM이 발신 화면(최근 통화 상단의 `NPC 전화 발신`)에서 NPC Actor·대상 플레이어·장면을 골라 걸면 대상 플레이어에게만 소켓으로 전달된다. 통화 기록은 아직 클라이언트 세션 메모리에만 남아 새로고침하면 사라진다.
