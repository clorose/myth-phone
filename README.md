...
- 연락처: 실제 사용자 + NPC 연락처(Actor 플래그), GM 추가·수정·삭제, 메시지 버튼으로 대화방 연결
...
버블톡 1:1 대화는 실제 채팅이다. 친구 탭에 접속 사용자가 표시되고, 보낸 메시지는 `ChatMessage` 귓속말로 영구 저장되며 상대 화면에 실시간으로 나타난다. 채팅 탭에는 실제 대화가 위에, UI 확인용 더미 대화가 아래에 함께 표시된다(더미는 단체톡 실장 시 제거 예정). 기본 메시지 앱과 버블톡 단체 대화는 아직 JSON 더미 데이터다. 전화는 GM이 발신 화면(최근 통화 상단의 `NPC 전화 발신`)에서 NPC Actor·대상 플레이어·장면을 골라 걸면 대상 플레이어에게만 소켓으로 전달된다. 통화 기록은 아직 클라이언트 세션 메모리에만 남아 새로고침하면 사라진다.
...
1. 이미지·증거 자료 표시 (메시지·버블톡 안에서)
2. 메시지 앱(연출용 기록)을 GM이 편집 가능한 월드 데이터로 전환
3. 버블톡 더미 데이터 제거
4. NPC 연락처 노출 범위(아는 NPC만 표시)
5. 단체톡 초대·나가기
