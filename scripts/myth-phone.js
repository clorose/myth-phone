...
class SmartphoneShell {
  ...
  static gmEditorRow(id, initial, title, subtitle) {
    return `
  ...
  static async gmEditorDelete(content, kind, id) {
  ...
  static async gmEditorCreate(content, kind) {
  ...
  static renderGmEditorDetail(content, kind, id) {
    if (!game.user.isGM) { this.showHome(this.wrapper); return; }
    const list = foundry.utils.deepClone(this.editorData(kind));
  ...
  static renderGmEmailDetail(content, list, item) {
    content.closest(".smartphone-app-view")?.classList.remove("is-chat-open");
  ...
  static renderGmEditorExport(content, kind) {
  ...
  static renderGmEditorImport(content, kind) {
    content.innerHTML = `
      <header class="phone-page-header gm-editor-header gm-editor-detail">
        <button class="gm-editor-back" type="button" aria-label="뒤로"><i class="fa-solid fa-chevron-left"></i></button>
        <p>연출 편집 · 가져오기</p>
