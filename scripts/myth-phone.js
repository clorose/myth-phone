...
class SmartphoneShell {
  ...
  static renderBubbleTalk(content, section = "friends") {
    ...
    const conversations = section === "groups"
  ...
  static bubbleTalkFriendItems() {
    if (!this.messageData.bubbletalkFriends.length) {
  ...
  static renderBubbleTalkChat(content, conversationId) {
    const conversation = this.messageData.bubbletalk.find(
  ...
  static renderContacts(content) {
    ...
    content.innerHTML = `
      <header class="phone-page-header">
        <p>연락처</p><h2>연락처</h2>
        <button type="button" aria-label="연락처 추가"><i class="fa-solid fa-user-plus"></i></button>
      </header>
      <label class="phone-search">
        <i class="fa-solid fa-magnifying-glass"></i>
      ...
      <div class="phone-contact-list">
        ${contacts.map(([initial, name, number]) => `
          <article class="phone-contact" data-name="${name} ${number}">
            <span class="phone-avatar">${initial}</span>
            <span><strong>${name}</strong><small>${number}</small></span>
            <button type="button" aria-label="${name}에게 메시지"><i class="fa-solid fa-comment"></i></button>
          </article>`).join("")}
      </div>
    `;
