...
- **세션 시작 시 동기화 확인 후 `Compact.md`부터 읽는다** (세션 인수인계, git 미추적 — 열린 결정·잔무만 담는다).
- **질문에는 답부터.** 코드·문서 수정은 "고쳐"라는 지시가 있을 때만 한다.
- **실기 확인 상태의 정본은 `docs/TESTING.md` 체크박스 하나뿐.** PROGRESS·Compact 등 다른 곳에
  미확인 상태를 복제하지 않는다. 진행 상태·기능 유무 질문에는 암기로 답하지 말고 코드와 체크리스트를
  열어 확인 후 답한다.
- **UI 지적을 받으면 `../screen_shot/` 최신 파일부터 연다.** 실기 리뷰는 사용자가 거기에 스크린샷을
  올리고 말하는 방식이다.
- 스타일을 고쳤는데 화면이 그대로면 값 조정 말고 "규칙이 적용되긴 하는지"부터 의심한다.
- `docs/user/`는 읽어도 되지만 내용을 판단·제안의 근거로 쓰지 않는다.
